...
from examples.holder_table.problem import HolderTableProblem
from examples.holder_table.plotting import HolderTablePlotter
...
def main() -> None:
    ...
    out_dir = Path(f"examples/holder_table/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            ...
            description="HolderTable x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-10.0, 10.0),
            description="HolderTable y dimension",
            unit="units",
        ),
    ]
    ...
    mutation = UniformMutation(scale=0.5, prob=0.45, normalized=True)
    ...
    convergence = ConvergenceChecker[NDArrayFloat, float](
        strategies=[MaxEvaluationsStop(90_000)],
        iteration_strategy=MaxIterationsStop(260),
    )
    ...
    plotter = HolderTablePlotter(update_every=1, interactive=False)
