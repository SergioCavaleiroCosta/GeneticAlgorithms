"""Run NSGA-II on the ZDT2 multi-objective test problem."""
...
from examples.zdt2.problems import ZDT2Problem
from examples.zdt2.plotting import ParetoPlotter, MultiObjectivePopulationLogger, plot_pareto_front


def main() -> None:
    # Problem setup
    dimension = 10
    problem = ZDT2Problem(dimension=dimension)
    
    # Parameters (all in [0,1] for ZDT2)
    parameters = [
        ContinuousParameter(
            name=f"x{i}",
            normalizer=LinearNormalization(0.0, 1.0),  # ZDT2 domain is [0,1]
            description=f"ZDT2 dimension {i}",
            unit="units",
        )
    ...
    out_dir = Path(f"examples/zdt2/output_{timestamp}")
