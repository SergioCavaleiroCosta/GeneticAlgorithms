"""Run NSGA-II on the ZDT3 multi-objective test problem."""
...
from examples.zdt3.problems import ZDT3Problem
from examples.zdt3.plotting import ParetoPlotter, MultiObjectivePopulationLogger, plot_pareto_front


def main() -> None:
    # Problem setup
    dimension = 10
    problem = ZDT3Problem(dimension=dimension)
    
    # Parameters (all in [0,1] for ZDT3)
    parameters = [
        ContinuousParameter(
            name=f"x{i}",
            normalizer=LinearNormalization(0.0, 1.0),  # ZDT3 domain is [0,1]
            description=f"ZDT3 dimension {i}",
            unit="units",
        )
    ...
    out_dir = Path(f"examples/zdt3/output_{timestamp}")
