...
from examples.cross_in_tray.problem import CrossInTrayProblem
from examples.cross_in_tray.plotting import CrossInTrayPlotter
from optimization.events import PopulationLogger, FrameSaverStrategy, FrameSaverConfig


def main() -> None:
    problem = CrossInTrayProblem()

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(f"examples/cross_in_tray/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            ...
            description="CrossInTray x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-10.0, 10.0),
            description="CrossInTray y dimension",
            unit="units",
        ),
    ]
    ...
    mutation = UniformMutation(scale=0.45, prob=0.4, normalized=True)
    ...
    plotter = CrossInTrayPlotter(update_every=1, interactive=False)
