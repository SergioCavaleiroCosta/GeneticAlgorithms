...
from examples.mccormick.problem import McCormickProblem
from examples.mccormick.plotting import McCormickPlotter
from optimization.events import PopulationLogger, FrameSaverStrategy, FrameSaverConfig


def main() -> None:
    problem = McCormickProblem()

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(f"examples/mccormick/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            name="x",
            normalizer=LinearNormalization(-1.5, 4.0),
            description="McCormick x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-3.0, 4.0),
            description="McCormick y dimension",
            unit="units",
        ),
    ]
    ...
    mutation = UniformMutation(scale=0.3, prob=0.3, normalized=True)
    ...
    convergence = ConvergenceChecker[NDArrayFloat, float](
        strategies=[MaxEvaluationsStop(55_000)],
        iteration_strategy=MaxIterationsStop(180),
    )
    ...
    plotter = McCormickPlotter(update_every=1, interactive=False)
