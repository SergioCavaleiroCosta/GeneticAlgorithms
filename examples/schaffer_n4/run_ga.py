...
from examples.schaffer_n4.problem import SchafferN4Problem
from examples.schaffer_n4.plotting import SchafferN4Plotter
from optimization.events import PopulationLogger, FrameSaverStrategy, FrameSaverConfig


def main() -> None:
    problem = SchafferN4Problem()

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(f"examples/schaffer_n4/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            name="x",
            normalizer=LinearNormalization(-50.0, 50.0),
            description="SchafferN4 x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-50.0, 50.0),
            description="SchafferN4 y dimension",
            unit="units",
        ),
    ]
    ...
    mutation = UniformMutation(scale=0.35, prob=0.35, normalized=True)
    ...
    convergence = ConvergenceChecker[NDArrayFloat, float](
        strategies=[MaxEvaluationsStop(70_000)],
    ...
    plotter = SchafferN4Plotter(update_every=1, interactive=False)
