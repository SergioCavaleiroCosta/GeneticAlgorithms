...
from examples.matyas.problem import MatyasProblem
from examples.matyas.plotting import MatyasPlotter
from optimization.events import PopulationLogger, FrameSaverStrategy, FrameSaverConfig


def main() -> None:
    problem = MatyasProblem()

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(f"examples/matyas/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            ...
            description="Matyas x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-10.0, 10.0),
            description="Matyas y dimension",
            unit="units",
        ),
    ]
    ...
    mutation = UniformMutation(scale=0.25, prob=0.25, normalized=True)
    ...
    convergence = ConvergenceChecker[NDArrayFloat, float](
        ...
        iteration_strategy=MaxIterationsStop(80),
    )
    ...
    plotter = MatyasPlotter(update_every=1, interactive=False)
