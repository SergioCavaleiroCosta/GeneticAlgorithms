...
from examples.booth.problem import BoothProblem
from examples.booth.plotting import BoothPlotter
...
def main() -> None:
    ...
    out_dir = Path(f"examples/booth/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            ...
            description="Booth x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-10.0, 10.0),
            description="Booth y dimension",
            unit="units",
        ),
    ]
    ...
    convergence = ConvergenceChecker[NDArrayFloat, float](
        strategies=[MaxEvaluationsStop(30_000)],
    ...
    plotter = BoothPlotter(update_every=1, interactive=False)
