...
from examples.schaffer_n2.problem import SchafferN2Problem
from examples.schaffer_n2.plotting import SchafferN2Plotter
from optimization.events import PopulationLogger, FrameSaverStrategy, FrameSaverConfig


def main() -> None:
    problem = SchafferN2Problem()

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(f"examples/schaffer_n2/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            ...
            description="SchafferN2 x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-50.0, 50.0),
            description="SchafferN2 y dimension",
            unit="units",
        ),
    ]
    ...
    plotter = SchafferN2Plotter(update_every=1, interactive=False)
