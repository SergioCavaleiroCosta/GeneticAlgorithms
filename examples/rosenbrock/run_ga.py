...
from examples.rosenbrock.problem import RosenbrockProblem
from examples.rosenbrock.plotting import RosenbrockPlotter
from optimization.events import PopulationLogger, FrameSaverStrategy, FrameSaverConfig


def main() -> None:
    dimension = 10
    problem = RosenbrockProblem(dimension=dimension)

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(f"examples/rosenbrock/output_{timestamp}")
    figures_dir = out_dir / "figures"
    out_dir.mkdir(parents=True, exist_ok=True)
    figures_dir.mkdir(parents=True, exist_ok=True)

    # Typical domain reduces to tighter region around optimum; use [-2.0, 2.0]
    parameters = [
        ContinuousParameter(
            name=f"x{i}",
            normalizer=LinearNormalization(-2.0, 2.0),
            description=f"Rosenbrock dimension {i}",
            unit="units",
        )
    ...
    convergence = ConvergenceChecker[NDArrayFloat, float](
        strategies=[MaxEvaluationsStop(150_000)],
    ...
    plotter = RosenbrockPlotter(update_every=1, param_pair=(0, 1), interactive=False)
