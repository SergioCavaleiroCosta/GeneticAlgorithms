...
from examples.bukin6.problem import Bukin6Problem
from examples.bukin6.plotting import Bukin6Plotter
...
def main() -> None:
    problem = Bukin6Problem()

    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = Path(f"examples/bukin6/output_{timestamp}")
    ...
    parameters = [
        ContinuousParameter(
            name="x",
            normalizer=LinearNormalization(-15.0, -5.0),
            description="Bukin6 x dimension",
            unit="units",
        ),
        ContinuousParameter(
            name="y",
            normalizer=LinearNormalization(-3.0, 3.0),
            description="Bukin6 y dimension",
            unit="units",
        ),
    ]
    ...
    convergence = ConvergenceChecker[NDArrayFloat, float](
        strategies=[MaxEvaluationsStop(60_000)],
    ...
    plotter = Bukin6Plotter(update_every=1, interactive=False)
